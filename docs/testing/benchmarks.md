...
- **Megapixels per second (MP/s)**: $\frac{\text{Pixels Processed}}{10^6 \times \Delta t_{\text{sec}}}$
- **Megabytes per second (MB/s)**: $\frac{\text{Bytes Processed}}{10^6 \times \Delta t_{\text{sec}}}$
- **Latency Percentiles**: Average, p50, and p95 execution time in milliseconds.
- **Heap Memory Delta**: Peak Resident/Heap usage during execution, ensuring compliance with streaming $O(\text{tile})$ / $O(\text{scanline})$ memory constraints.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Prepress Resampling** (Bicubic Downsampling) | $800 \times 800 \to 400 \times 400$ | **15.60 MP/s** | **44.62 MB/s** | 41.0 ms / 41.3 ms | $+0.00\text{ MB}$ |
| **Color Management** (sRGB $\to$ CMYK 3D LUT + TAC 300%) | $400 \times 400$ RGBA8888 | **2.91 MP/s** | **8.33 MB/s** | 55.0 ms / 55.2 ms | $+10.18\text{ MB}$ |
| **Layer Compositing & Flattening** (5 layers) | $500 \times 500$ RGBA8888 | **740.66 MP/s** | **2825.41 MB/s** | 1.69 ms / 1.81 ms | $+0.00\text{ MB}$ |
| **Camera RAW Demosaicing** (Bayer RGGB) | $1000 \times 1000$ sensels (1 MP) | **5.92 MP/s** | **11.29 MB/s** | 168.9 ms / 173.4 ms | $+0.00\text{ MB}$ |
| **Plate Separation** (`tiffsep` CMYK Plates) | $600 \times 600$ CMYK8888 | **63.12 MP/s** | **240.80 MB/s** | 5.70 ms / 5.83 ms | $+0.17\text{ MB}$ |
...
2. **Memory Growth**: Peak heap delta must remain $< 256\text{ MB}$ across all benchmark operations. Zero unbounded heap growth is permitted.
