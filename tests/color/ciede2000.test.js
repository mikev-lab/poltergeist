...
import { LabColor } from '../../src/types/color.js';

describe('CIEDE2000 Color Difference Metric (Sharma et al. 2005 Reference)', () => {
  test('Identical colors yield Delta E 00 = 0.0', () => {
    const lab = new LabColor(50.0, 25.0, -15.0);
    const de = ciede2000(lab, lab);
    assert.strictEqual(de, 0.0);
  });

  // Authoritative 34-vector benchmark from Gaurav Sharma, Wencheng Wu, Edul N. Dalal (2005)
  ...
  sharmaDataset.forEach((row, i) => {
    test(`Sharma vector pair #${i + 1}`, () => {
      const c1 = new LabColor(row[0], row[1], row[2]);
      const c2 = new LabColor(row[3], row[4], row[5]);
      const expected = row[6];
      const de = ciede2000(c1, c2);
      const diff = Math.abs(de - expected);
      assert(
        diff < 0.0001,
        `Pair #${i + 1}: expected ${expected}, got ${de.toFixed(4)} (diff: ${diff.toFixed(6)})`
      );
    });
  });
});
