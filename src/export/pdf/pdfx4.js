...
export class PdfX4Generator {
  /**
   * Generates a valid PDF/X-4:2010 file from a RasterImage, Document, or PageRecord array.
  ...
  static generate(input, options = {}) {
    ...
    if (input instanceof Document) {
      ...
      throw new TypeError('Unsupported input type for PDF/X-4 generation');
    }

    if (pages.length === 0) {
      throw new Error('Cannot generate PDF/X-4 document with zero pages');
    }
    ...
    for (let pIdx = 0; pIdx < pages.length; pIdx++) {
      ...
      if (page.rasterBackground) {
        ...
        } else if (img.colorSpace === ColorSpaceType.GRAY) {
      ...
      if (page.paths && page.paths.length > 0) {
        contentLines.push('q', '0 0 0 RG', '0 0 0 rg', '1 w'); // RGB black stroke/fill
